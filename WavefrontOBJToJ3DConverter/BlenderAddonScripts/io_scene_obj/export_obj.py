...
def write_file(filepath, objects, depsgraph, scene,
               ...
               EXPORT_RENAME_OBJ_WITH_HIERARCHY_INFO=True,
                ...
                for ob, ob_mat in obs:
                    with ProgressReportSubstep(subprogress1, 6) as subprogress2:
                        ...
                        if EXPORT_BLEN_OBS or EXPORT_GROUP_BY_OB:
                            ...
                            cur_ob = ob
                            hierarchy_name = ob.name
                            while cur_ob.parent is not None:
                                cur_ob = cur_ob.parent
                                hierarchy_name = cur_ob.name + "." + hierarchy_name

                            local_translation = ob.matrix_local.to_translation()[:]
                            if EXPORT_BLEN_OBS:
                                if EXPORT_RENAME_OBJ_WITH_HIERARCHY_INFO:
                                    fw('o %s(%f,%f,%f)\n' % (hierarchy_name, local_translation[0], local_translation[1], local_translation[2]))  # Write Object name
                                else:
                                    fw('o %s\n' % obnamestring)
...
def _write(context, filepath,
           ...
           EXPORT_RENAME_OBJ_WITH_HIERARCHY_INFO,
           ):
...
def save(context,
         ...
         *,
         ...
         global_matrix=None,
         path_mode='AUTO',
         rename_obj_with_hierarchy_info=True,
         ):
    ...
    _write(context, filepath,
           ...
           EXPORT_RENAME_OBJ_WITH_HIERARCHY_INFO=rename_obj_with_hierarchy_info,
           )
